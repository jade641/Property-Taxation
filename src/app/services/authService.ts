...
function normalizeAuthError(error: unknown): never {
  ...
  if (error instanceof AxiosError) {
    if (error.code === 'ECONNABORTED') {
      throw new AuthRequestError(error.message || 'Login request timed out. Please try again.')
    }
...
export async function forgotPasswordRequest(payload: ForgotPasswordPayload): Promise<string> {
  try {
    ...
    if (!response.data.success) {
      throw new AuthRequestError(response.data.message || 'Unable to send OTP.', response.data.errors ?? [])
    }
...
export async function verifyOtpRequest(payload: VerifyOtpPayload): Promise<string> {
  try {
    const response = await api.post<ApiResponse<null>>('/auth/verify-otp', payload)

    if (!response.data.success) {
      throw new AuthRequestError(response.data.message || 'Invalid verification code.', response.data.errors ?? [])
    }
...
export async function resetPasswordRequest(payload: ResetPasswordPayload): Promise<string> {
  try {
    ...
    if (!response.data.success) {
      throw new AuthRequestError(response.data.message || 'Password reset failed.', response.data.errors ?? [])
    }
...
export function updateStoredAuthUser(user: AuthenticatedUser) {
  if (typeof window === 'undefined') {
    return
  }
