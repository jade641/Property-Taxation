    ...
    (async () => {
      try {
        ...
        setForm({
          ...
          position: profile.position ?? (user!.role === "Admin" ? "System Administrator" :
                    user!.role === "Accountant" ? "Revenue Collection Officer" :
                    user!.role === "Staff" ? "Data Entry Specialist" : "Internal Auditor"),
          municipality: profile.municipality ?? "Davao City",
        });
      } catch (e) {
        // don't block UI; surface non-fatal error in banner area
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            { label: "Backup Email",key: "backupEmail",  type: "email", placeholder: "Optional recovery email" },
              ...
              onClick={async () => {
                ...
                try {
                  await updateMyProfile({
                    fullName: form.name,
                    email: form.email,
                    backupEmail: form.backupEmail || null,
