...
import { getLandingSummary, type LandingSummary } from "../services/landingService";
...
function formatMonthLabel(label: string | null) {
  if (!label) {
    return "N/A";
  }

  const parsed = new Date(`${label}-01T00:00:00`);
  return Number.isNaN(parsed.getTime()) ? label : parsed.toLocaleString("en-PH", { month: "short", year: "numeric" });
}
...
const MODULES = [
  ...
  { icon: Home,            name: "Property Registry",       desc: "Register and manage all real properties by barangay, type, and assessed value.", color: "indigo"  },
  ...
  { icon: FileText,        name: "Government Reporting",    desc: "Generate barangay summaries, monthly collections, and annual LGU filings.", color: "orange"  },
  ...
  { icon: Users,           name: "User Management",         desc: "Manage accounts, assign roles, and set access levels.", color: "purple"  },
];
...
const ROLES = [
  ...
  { name: "Staff (Encoder)",level: 2, modules: 5, desc: "Dashboard overview, property registration, tax calculation, payment recording, and filing access.", badge: "bg-slate-100 text-slate-700 border-slate-200",     bar: "bg-slate-500"  },
    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-16 grid grid-cols-3 items-center">
        ...
        <div className="flex items-center gap-3">
          <BrandLogo
            variant="mark"
            className={`w-12 h-12 flex-shrink-0 ring-1 shadow-sm ${scrolled ? "ring-slate-200" : "ring-white/10"}`}
          />
          <div>
            <p className={`text-base font-bold tracking-tight leading-none ${scrolled ? "text-slate-900" : "text-white"}`}>
              TaxSync
            </p>
            <p className={`text-[11px] leading-tight ${scrolled ? "text-slate-500" : "text-blue-300"}`}>
              Property Tax System
            </p>
          </div>
        </div>

        {/* Desktop links — center col, truly centered */}
        <nav className="hidden md:flex items-center justify-center gap-6 -translate-x-2">
        ...
        <div className="hidden md:flex items-center justify-end gap-3">
          <button
            ...
            style={scrolled
              ...
              : { backgroundColor: "rgba(255,255,255,0.15)", color: "#ffffff", border: "1px solid rgba(255,255,255,0.25)", backdropFilter: "blur(8px)" }}
          >
            Sign In to Portal
          </button>
        </div>

        {/* Mobile hamburger — right col */}
...
// ─── Hero Section ─────────────────────────────────────────────────────────────
function Hero({ summary, loading }: { summary: LandingSummary | null; loading: boolean }) {
  const hasSummary = summary !== null;
  const latestCollectionLabel = formatMonthLabel(summary?.latestCollectionLabel ?? null);
  const collectedAmount = summary?.totalCollected ?? 0;
  const outstandingBalance = summary?.totalOutstandingBalance ?? 0;
  const propertyCount = summary?.totalProperties ?? 0;
  const userCount = summary?.totalUsers ?? 0;
  const assessmentCount = summary?.totalAssessments ?? 0;
  ...
  const renderCount = (value: number) => (hasSummary ? value.toLocaleString() : "Unavailable");
  const renderCurrency = (value: number) => (hasSummary ? formatCurrency(value) : "Unavailable");
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <p className="text-lg leading-relaxed mb-8" style={{ color: "#93b8d8", maxWidth: "52ch" }}>
              A comprehensive enterprise ERP platform for local government units — streamlining real property tax administration,
              compliance monitoring, and official government reporting under R.A. 7160, powered by live database records.
            </p>
          ...
          >
            <div className="rounded-2xl overflow-hidden shadow-2xl"
              ...
              <div className="flex items-center gap-3 px-4 py-3" style={{ backgroundColor: "rgba(0,0,0,0.3)" }}>
                <div className="w-3 h-3 rounded-full bg-emerald-500/70" />
                <div>
                  <p className="text-[11px] font-semibold text-white">Live database snapshot</p>
                  <p className="text-[10px]" style={{ color: "#8aa6c5" }}>
                    {loading ? "Loading current records..." : "Values are pulled from the current users and records in the database."}
                  </p>
                </div>
                <div className="ml-auto text-right">
                  <p className="text-[10px] text-blue-300 font-semibold uppercase tracking-widest">Latest collections</p>
                  <p className="text-[11px] text-white">{latestCollectionLabel}</p>
                </div>
              </div>
              ...
              <div className="p-5" style={{ backgroundColor: "#f8fafc" }}>
                <div className="grid grid-cols-4 gap-2 mb-3">
                  {[
                    { label: "Properties", value: propertyCount, sub: "Registered in DB", color: "text-blue-600", bg: "bg-blue-50" },
                    { label: "Users", value: userCount, sub: "Active portal accounts", color: "text-indigo-600", bg: "bg-indigo-50" },
                    { label: "Assessments", value: assessmentCount, sub: "Recorded assessments", color: "text-amber-600", bg: "bg-amber-50" },
                    { label: "Compliance", value: `${complianceRate.toFixed(1)}%`, sub: "Live status from data", color: "text-emerald-600", bg: "bg-emerald-50" },
                  ].map((k) => (
                    <div key={k.label} className={`${k.bg} rounded-lg p-2.5`}>
                      ...
                      <p className={`text-sm font-bold mt-0.5 ${k.color}`}>
                        {typeof k.value === "number" ? renderCount(k.value) : (hasSummary ? k.value : "Unavailable")}
                      </p>
                ...
                <div className="bg-white rounded-lg p-3 mb-2 border border-slate-200">
                  <p className="text-[10px] font-semibold text-slate-600 mb-2">Real-time collection summary</p>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="rounded-md border border-slate-200 bg-slate-50 p-2">
                      <p className="text-[9px] uppercase tracking-wider text-slate-400">Collected</p>
                      <p className="text-sm font-bold text-emerald-700">{renderCurrency(collectedAmount)}</p>
                    </div>
                    <div className="rounded-md border border-slate-200 bg-slate-50 p-2">
                      <p className="text-[9px] uppercase tracking-wider text-slate-400">Outstanding</p>
                      <p className="text-sm font-bold text-red-700">{renderCurrency(outstandingBalance)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  <div className="flex-1 bg-emerald-50 border border-emerald-200 rounded-lg p-2 text-center">
                    ...
                    <p className="text-sm font-bold text-emerald-700">{hasSummary ? compliantCount : "Unavailable"}</p>
                  </div>
                  <div className="flex-1 bg-amber-50 border border-amber-200 rounded-lg p-2 text-center">
                    ...
                    <p className="text-sm font-bold text-amber-700">{hasSummary ? lateCount : "Unavailable"}</p>
                  </div>
                  <div className="flex-1 bg-red-50 border border-red-200 rounded-lg p-2 text-center">
                    ...
                    <p className="text-sm font-bold text-red-700">{hasSummary ? unpaidCount : "Unavailable"}</p>
                  </div>
                </div>
              </div>
            </div>
...
// ─── Stats Section ────────────────────────────────────────────────────────────
function StatsSection({ summary, loading }: { summary: LandingSummary | null; loading: boolean }) {
  const hasSummary = summary !== null;
  const stats = [
    { value: summary?.totalProperties ?? 0, suffix: "", label: "Properties Registered", icon: Home, color: "text-blue-300" },
    { value: summary?.totalCollected ?? 0, suffix: "", label: "Tax Collected YTD", icon: TrendingUp, color: "text-emerald-300", prefix: "₱" },
    { value: summary?.complianceRate ?? 0, suffix: "%", label: "Compliance Rate", icon: ShieldCheck, color: "text-amber-300" },
    { value: summary?.totalUsers ?? 0, suffix: "", label: "Registered Users", icon: Users, color: "text-violet-300" },
  ];
            ...
            return (
              <div key={s.label} className="text-center">
                ...
                <p className="text-white mb-1" style={{ fontSize: "1.85rem", fontWeight: 800, letterSpacing: "-0.02em" }}>
                  {loading ? "Loading..." : hasSummary ? <Counter to={s.value} prefix={s.prefix} suffix={s.suffix} /> : "Unavailable"}
                </p>
...
function FeaturesSection() {
  const highlights = [
    ...
    { icon: Zap,      title: "AI Anomaly Detection",        desc: "Machine learning model trained on live property assessment and payment records flags assessed values outside normal range for immediate audit review.", color: "violet"  },
    ...
    { icon: Building2, title: "Multi-Barangay Support",     desc: "Manage properties across all barangays with breakdown charts, compliance summaries, and collection efficiency reports per district.",              color: "orange"  },
  ];
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <h2 className="text-slate-900 mb-4" style={{ fontSize: "clamp(1.6rem, 3vw, 2.4rem)", fontWeight: 800 }}>
            Built for Philippine LGU<br />Property Tax Administration
          </h2>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <p className="text-slate-500 leading-relaxed mb-6">
              TaxSync is designed in full compliance with Republic Act 7160 (Local Government Code), DILG Memorandum Circulars on RPT administration, and COA audit standards for LGU financial systems.
            </p>
            ...
            <div className="relative z-10 p-8">
              ...
              <div className="space-y-4">
                {[
                  ...
                  { label: "Access",        value: "Authorized LGU Personnel Only", icon: Users  },
    ...
    >
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
        ...
        <p className="mb-8 leading-relaxed" style={{ color: "#93b8d8", fontSize: "1.0625rem" }}>
          Use your authorized LGU credentials to sign in. Contact your system administrator if you require account provisioning.
        </p>
        ...
        <p className="mt-6 text-xs" style={{ color: "#4a7fa5" }}>
          For authorized LGU personnel only · v3.1.0 Enterprise · © 2026 TaxSync
        </p>
      </div>
    </section>
  );
    ...
    <footer className="bg-white border-t border-slate-200 py-10">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-3 items-center gap-6">
          {/* Left col — logo (matches navbar logo col) */}
          <div className="flex items-center gap-3">
            <BrandLogo variant="mark" className="w-8 h-8 rounded-lg ring-1 ring-slate-200 shadow-sm" />
            <div>
          ...
          {/* Center col — links (matches navbar nav col, same -translate-x-2 shift) */}
          <div className="flex items-center justify-center gap-6 -translate-x-2">
            ...
            <a href="#roles"    className="text-xs text-slate-400 hover:text-slate-600 transition-colors">Roles</a>
            <a href="#about"    className="text-xs text-slate-400 hover:text-slate-600 transition-colors">Legal</a>
          </div>
          {/* Right col — copyright (matches navbar CTA col) */}
          <p className="text-xs text-slate-400 text-right">
            © 2026 Republic of the Philippines · Department of Finance · LGU Division
          </p>
        </div>
      </div>
    </footer>
...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    async function loadSummary() {
      try {
        const data = await getLandingSummary();
